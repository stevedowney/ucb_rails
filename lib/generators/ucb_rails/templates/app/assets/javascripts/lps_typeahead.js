$(function() {
  ...
  // prevent browser autocomplete from competing with type-ahead
  $('.lps-typeahead-search').attr('autocomplete', 'off');
  
  // typing in search field clears uid field
  $('.lps-typeahead-search').keyup(function() {
    var uidSelector = '#' + $(this).data('uid-dom-id');
    $(uidSelector).val('');
  });
  
  // default handler for ldap search result link when used with typeahead
  $(document).on('click', 'a.lps-typeahead-itemx', function (e) {
    var link = $(this);
  ...
  $('.lps-typeahead-search').typeahead({
